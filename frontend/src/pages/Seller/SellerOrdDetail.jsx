...
export default function SellerOrderDetailPage() {
  ...
  // ✅ ánh xạ trạng thái → badge
  const getStatusBadge = (status) => {
    const base = "px-3 py-1 rounded-full font-semibold";
    switch (status) {
      case "pending":
        return `${base} bg-yellow-500 text-gray-900`;
      case "processing":
  ...
  const getNextStatuses = (status) => {
    switch (status) {
      ...
      default:
        return []; // completed hoặc canceled thì không đổi được
    }
  };

  if (loading)
    return <p className="text-center mt-10 text-gray-300">Đang tải...</p>;
  if (!order)
    return <p className="text-center mt-10 text-red-500">Không tìm thấy đơn hàng</p>;
      ...
      <div className="p-6 bg-gray-900 rounded-2xl shadow-lg flex-1 text-gray-100">
        ...
        <div className="mb-6 p-4 bg-gray-800 rounded-lg shadow-inner space-y-2">
          <p>
            ...
            <span className="text-blue-400">{order.user.username}</span>
          </p>
          <p>
          ...
          <div className="mt-2 flex items-center gap-2">
            ...
            {updating && (
              <span className="ml-2 text-gray-400">Đang cập nhật...</span>
            )}
          </div>
        </div>
